...
class FaturaController extends Controller
{
    ...
    public function listar()
    {
        $dados['faturas'] = DB::table('faturas')
            ...
            ->select(
                'faturas.id',
                'faturas.it_validacao_fatura',
                'faturas.vc_descricao',
                'publicidades.vc_nomePublicidade',
                'faturas.dt_pagamento',
                'faturas.dt_licenca',
                'faturas.fl_total',
                'faturas.it_id_subscricao'
            )
            ->where('faturas.it_estado', 1)
            ->get();

        return view('admin.fatura.listar.index', $dados);
    }
    ...
    public function gerarRelatorioLiquidacao($id,$dias)
    {
        $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => [210, 297]]);
        $data = date('d-m-Y');
        $total = 0;
